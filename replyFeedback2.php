...
<?php
	...
	$ID = $_GET['ID'];
	$courierReply = $_GET['courierReply'];
	
	$sql = "SELECT * FROM feedback WHERE ID = '$ID'";
	$result = mysqli_query($combine, $sql);
	$row= mysqli_fetch_array($result, MYSQLI_ASSOC);
	
	$q1 = "UPDATE feedback SET courierReply='$courierReply' WHERE feedback.ID = '$ID'";
	...
	if($combine->query($q1)===TRUE)
	{
		try 
		{
			...
			//Recipients
			$mail->setFrom('from@example.com', 'Courier Reply Feedback');
			$mail->addAddress('lyy395989@gmail.com');  //Add a recipient
			
			//Email notification content
			$message="Courier Reply Feedback : ".$courierReply;
			
			//Content
			$mail->isHTML(true); //Set email format to HTML
			$mail->Subject = 'Courier Reply Feedback';
			$mail->Body    = $message;
		...
		window.location='viewFeedback.php'</script>";
	}
